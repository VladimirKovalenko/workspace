...
//		}
	...
	public static void test18() {
		...
		for (int i = 0; i < masSort.length; i++) {
			for (int j = 0; j < masSort[i].length; j++) {
				System.out.print(masSort[i][j]+"\t");
			}
			System.out.println();
		}
	}
...
//		int masj[]={2,3,4,5,6,7,8,9};;
		...
		for (int i = 0; i < masRes.length; i++) {
			...
			for (int x = 0; x< masRes.length; x++) {
				if((ii==masRes[x][0] && jj==masRes[x][1]) || 
				   (jj==masRes[x][0] && ii==masRes[x][1]))
				{
					i--;
					flag=false;
					break;
